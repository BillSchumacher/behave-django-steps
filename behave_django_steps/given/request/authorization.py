...
def step_impl(context, group_name):
    ...
    context.execute_steps(f"""Given a "Group" with values
                          | name |
                          | {group_name} |""")
...
@given('The group "{group_name}" has the permission "{permission_name}"'
       ' with codename "{codename}" for model "{model_name}"')
def step_impl(context, group_name, permission_name, codename, model_name):
    """
    Args:
        context: behave.runner.Context
        group_name: str
    ...
    context.execute_steps(f'Given the permission "{permission_name}" with codename "{codename}" '
...
@given('the permission "{permission_name}" with codename "{codename}" '
       'for model "{model_name}" exists')
def step_impl(context, permission_name, codename, model_name):
    """
    Args:
        context: behave.runner.Context
        permission_name: str
